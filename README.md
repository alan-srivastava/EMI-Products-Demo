...
1) Prepare production builds and environment variables

- Frontend: run `npm run build` in `frontend` and deploy the resulting build to Vercel/Netlify or serve from a static host.
- Backend: deploy `backend` to a service like Render or Heroku. Configure environment variables on the host:
	- `MONGO_URI` — your MongoDB connection string
	- `PORT` — (default 5000)

2) Deploy backend to Render (example)
...
# Add environment variable MONGO_URI in the Render dashboard
```

3) Deploy frontend to Vercel (example)

Steps:

```bash
# Import project in Vercel and set Root to /frontend
# Build command: npm run build
# Output directory: dist
# Set env var VITE_API_URL to your backend URL (e.g. https://your-backend.onrender.com/api) if you want the frontend to call production API directly
```

Notes on CORS and images

- If frontend and backend are on different hosts in production, set `VITE_API_URL` to the full API base URL (including `/api`) and redeploy frontend.
- The seed images are stored in `frontend/public/images` — when frontend is hosted, those paths will resolve relative to frontend host.

Repository contents for submission

- `backend/models/Product.js` — schema file (already present)
- `backend/seed.js` — seed data (already present)
